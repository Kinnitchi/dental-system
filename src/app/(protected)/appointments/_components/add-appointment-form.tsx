...
const AddAppointmentForm = ({ patients, doctors, onSuccess, isOpen }: AddAppointmentFormProps) => {
  ...
  const { data: availableTimes } = useQuery({
    queryKey: ["available-times", selectedDate, selectedDoctorId, selectedPatientId],
    queryFn: () =>
      getAvailableTimes({
  ...
  const isDateAvailable = (date: Date) => {
    ...
    return (
      dayOfWeek >= Number(selectedDoctor.availableFromWeekDay) && dayOfWeek <= Number(selectedDoctor.availableToWeekDay)
    );
  };
  ...
  return (
    <DialogContent className="sm:max-w-125">
      <DialogHeader>
        ...
        <DialogDescription>Crie um novo agendamento para sua clínica.</DialogDescription>
      </DialogHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            render={({ field }) => (
              <FormItem>
                ...
                >
                  ...
                  <SelectContent>
                    {availableTimes?.data?.timeSlots?.map((time) => (
                      <SelectItem key={time.value} value={time.value} disabled={!time.available}>
          ...
          <DialogFooter>
            <Button type="submit" disabled={createAppointmentAction.isPending}>
              {createAppointmentAction.isPending ? "Criando..." : "Criar agendamento"}
            </Button>
          </DialogFooter>
        </form>
      </Form>
